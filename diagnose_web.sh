#!/usr/bin/env bash
# diagnose_web.sh — Diagnose "wrong HTML being served" on the Pi.
#
...
#
#   chmod +x ~/fermentatorium/diagnose_web.sh
#   ./diagnose_web.sh
#
...
hdr "2. Running Python processes (look for app.py, app.py, flask)"
echo ""
...
if [ -z "$py_procs" ]; then
  ...
  if echo "$py_procs" | grep -q "app.py"; then
    ok "app.py is running — this is the correct process."
  fi
fi
...
if command -v curl &>/dev/null; then
  ...
  if [ -z "$curl_headers" ]; then
    ...
    # Check for the identifying header stamped by app.py
    if echo "$curl_headers" | grep -qi "X-Fermentatorium-Server"; then
      ok "X-Fermentatorium-Server header present — response is from app.py."
      echo "$curl_headers" | grep -i "X-Fermentatorium-Server" | sed 's/^/    /'
    else
      fail "X-Fermentatorium-Server header MISSING — response is NOT from app.py!"
...
if command -v curl &>/dev/null; then
  ...
  if [ -z "$info_json" ]; then
    ...
    if [ -n "$srv_file" ]; then
      if echo "$srv_file" | grep -q "app.py"; then
        ok "Server file is app.py: $srv_file"
      else
        fail "Server file is NOT app.py: $srv_file — wrong version is running!"
      fi
    fi
  fi
else
