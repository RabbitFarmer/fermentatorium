#!/usr/bin/env python3
"""
a4app.py - Three Controller main Flask application.
...
try:
    from a4tilt_static import TILT_UUIDS, COLOR_MAP
except Exception:
    TILT_UUIDS = {}
    COLOR_MAP = {}

try:
    from a4kasa_worker import kasa_worker, kasa_query_state
except Exception:
    kasa_worker = None
...
try:
    from a4logger import log_error, log_kasa_command, log_notification, log_event
except Exception:
    def log_error(msg):
...
print(f"[fermentatorium] a4app.py  : {os.path.join(_HERE, 'a4app.py')}")
...
# --- Stop other a4app.py processes on startup --------------------------------
def stop_other_app_py():
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    if any('a4app.py' in str(p) for p in cmdline) or 'a4app.py' in name:
                        try:
                            proc.terminate()
                            proc.wait(timeout=5)
                            stopped.append(pid)
    ...
    try:
        pgrep = subprocess.run(['pgrep', '-f', 'a4app.py'], capture_output=True, text=True)
    ...
    while True:
        try:
            ...
            for ctrl in temp_cfg.get('controllers', []):
                if mode == 'heating' and url == ctrl.get("heating_plug"):
                    controller = ctrl
                    break
                elif mode == 'cooling' and url == ctrl.get("cooling_plug"):
                    controller = ctrl
                    break
            
            if controller is None:
                print(f"[KASA_RESULT] WARNING: Could not find controller for {mode} plug at {url}")
                continue
...
def dashboard():
    ...
    return render_template('a4maindisplay.html',
...
def startup():
    ...
    return render_template('a4startup.html')
...
def system_config():
    ...
    return render_template('a4system_config.html', 
                         system_settings=system_cfg,
...
def tilt_config():
    ...
    return render_template('a4tilt_config.html',
        tilt_cfg=tilt_cfg,
...
def batch_settings():
    ...
    return render_template('a4batch_settings.html',
        tilt_cfg=tilt_cfg,
...
def temp_config():
    ...
    return render_template('a4temp_control_config.html',
...
def temp_report():
    ...
    if not tilt_color:
        ...
        return render_template('a4temp_report_select.html', colors=colors, default_color=default_color)
    ...
    return render_template('a4temp_report_display.html',
                           color=tilt_color,
                           page=page,
...
def batch_history():
    ...
    return render_template('a4batch_history_select.html',
...
def batch_review(brewid):
    ...
    return render_template('a4batch_review.html',
                         batch=batch_info,
                         color=color,
...
def batch_data_view(brewid):
    ...
    return render_template('a4batch_data_view.html',
                         batch=batch_info,
                         color=color,
                         samples=all_samples,
                         stats=stats,
...
def scan_kasa_plugs():
    ...
    return render_template("a4kasa_scan_results.html", devices=devices, error=None,
...
def temp_summary(controller_id):
    ...
    return render_template('a4temp_summary.html',
...
def chart_plotly_index():
    ...
    return render_template('a4chart_plotly.html', tilt_color=None, system_settings=system_cfg)
...
def log_management():
    ...
    try:
        ...
        return render_template('a4log_management.html',
...
def view_log():
    ...
    try:
        ...
        return render_template('a4view_log.html',
                             log_file=log_file,
                             log_type=log_type,
                             content=content,
...
def exit_system():
    ...
    if request.method == 'POST':
        ...
        if confirm == 'yes':
            ...
            # Show goodbye page
            response = make_response(render_template('a4goodbye.html', brewery_name=system_cfg.get('brewery_name', 'ThreeControl')))
    ...
    return render_template('a4exit_system.html')
...
if __name__ == '__main__':
        ...
        if stopped_info.get('stopped'):
            ...
            if not wait_for_port_release(flask_port, flask_host, max_wait_seconds=10):
                ...
                if not attempt_to_free_port(flask_port, flask_host):
                    ...
                    print(f"[ERROR] Example: FLASK_PORT=5001 python3 a4app.py")
                    print(f"[ERROR] Or update 'flask_port' in {SYSTEM_CFG_FILE}")
                    sys.exit(1)
        else:
            # No a4app.py processes were stopped, but port is in use by something else
            ...
            if not attempt_to_free_port(flask_port, flask_host):
                ...
                print(f"[ERROR]   - Set environment variable: FLASK_PORT=5001 python3 a4app.py")
                print(f"[ERROR]   - Or update 'flask_port' in {SYSTEM_CFG_FILE}")
                sys.exit(1)
    ...
    # Skip if SKIP_BROWSER_OPEN is set (e.g., when running via a4start.sh)
