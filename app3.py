...
# --- Stop other app3.py processes on startup --------------------------------
def stop_other_app_py():
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    if any('app3.py' in str(p) for p in cmdline) or 'app3.py' in name:
                        try:
                            proc.terminate()
                            proc.wait(timeout=5)
                            stopped.append(pid)
    ...
    try:
        pgrep = subprocess.run(['pgrep', '-f', 'app3.py'], capture_output=True, text=True)
        ...
        if stopped_info.get('stopped'):
            ...
            if not wait_for_port_release(flask_port, flask_host, max_wait_seconds=10):
                ...
                if not attempt_to_free_port(flask_port, flask_host):
                    ...
                    print(f"[ERROR] Example: FLASK_PORT=5001 python3 app3.py")
                    print(f"[ERROR] Or update 'flask_port' in {SYSTEM_CFG_FILE}")
                    sys.exit(1)
        else:
            # No app3.py processes were stopped, but port is in use by something else
            ...
            if not attempt_to_free_port(flask_port, flask_host):
                ...
                print(f"[ERROR]   - Set environment variable: FLASK_PORT=5001 python3 app3.py")
                print(f"[ERROR]   - Or update 'flask_port' in {SYSTEM_CFG_FILE}")
                sys.exit(1)
